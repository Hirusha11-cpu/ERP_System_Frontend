...
const Account_receivable = () => {
  ...
  const fetchBankStatements = async () => {
    ...
    try {
      const response = await axios.get(
        `/api/bank-statements?company_id=${companyNo}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  return (
    <div className="account-receivable">
            ...
            <div>
              {activeTab === "credit" ? (
                <>
                  ...
                  >
                    <FaFileImport /> Upload Bank Statement
                  </Button>
                  <Button variant="outline-primary" size="sm" className="me-2 ms-3 mb-1" onClick={() => setShowUploadModal(true)}>
                    <FaFileImport /> Upload OS-Format
                  </Button>
                  <Button variant="primary" size="sm" onClick={fetchInvoices}>
                    <FaSyncAlt /> Refresh
                  </Button>
                </>
              ) : activeTab === "non-credit" ? (
                <>
                  <Button
                    variant="outline-primary"
                    size="sm"
            ...
            <Row>
              <Col md={12}>
                <Nav variant="tabs">
                  ...
                  <Nav.Item>
                    <Nav.Link eventKey="bank-statements">Bank Statements</Nav.Link>
                  </Nav.Item>
                </Nav>
                ...
                {loading ? (
                  ...
                  <Tab.Content>
                    ...
                    <Tab.Pane eventKey="credit">
                      {/* Existing Credit Partners table */}
                      <Table striped bordered hover responsive>
                        <thead>
                          <tr>
                            <th>Invoice #</th>
                    ...
                    <Tab.Pane eventKey="bank-statements">
                      <Table striped bordered hover responsive>
                        <thead>
                          <tr>
                        ...
                        <tbody>
                          {bankStatements.length > 0 ? (
                            bankStatements.map((statement) => (
                              <tr key={statement.id}>
                                <td>{new Date(statement.transaction_date).toLocaleDateString()}</td>
                                <td>{statement.transaction_remarks}</td>
                                <td>{statement.withdrawal_amt}</td>
                                <td>{statement.deposit_amt}</td>
                                <td>{statement.balance}</td>
                                <td>
                                  <Badge bg="info">{statement.credit_partner}</Badge>
                                </td>
                              </tr>
                            ))
                          ) : (
      ...
      <Modal show={showBankUploadModal} onHide={() => setShowBankUploadModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Upload Bank Statement CSV</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
            ...
            />
            <Form.Text className="text-muted">
              Upload a CSV bank statement to extract credit partner transactions.
            </Form.Text>
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowBankUploadModal(false)}>
            Cancel
          </Button>
          <Button
            variant="primary"
            onClick={handleBankUpload}
            disabled={!bankFile || loading}
          >
