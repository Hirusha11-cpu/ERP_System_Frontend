...
const Summary_report = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.account) {
      result = result.filter(invoice => invoice.account_id === parseInt(filters.account));
    }

    if (filters.startDate) {
      result = result.filter(invoice => 
        new Date(invoice.issue_date) >= new Date(filters.startDate)
      );
    }

    if (filters.endDate) {
      result = result.filter(invoice => 
        new Date(invoice.issue_date) <= new Date(filters.endDate)
      );
    }
    ...
    if (filters.customer) {
      result = result.filter(invoice => 
        invoice.customer?.name.toLowerCase().includes(filters.customer.toLowerCase())
      );
    }

    if (filters.paymentType) {
      result = result.filter(invoice => invoice.payment_type === filters.paymentType);
    }
  ...
  const calculateSummary = () => {
    ...
    filteredData.forEach(invoice => {
      ...
      summary.byCurrency[invoice.currency].amount += parseFloat(invoice.total_amount) || 0;
      summary.byCurrency[invoice.currency].received += parseFloat(invoice.amount_received) || 0;
      summary.byCurrency[invoice.currency].balance += parseFloat(invoice.balance) || 0;
      ...
      summary.byStatus[invoice.status].amount += parseFloat(invoice.total_amount) || 0;
      summary.byStatus[invoice.status].received += parseFloat(invoice.amount_received) || 0;
      summary.byStatus[invoice.status].balance += parseFloat(invoice.balance) || 0;
      ...
      summary.byAccount[accountName].amount += parseFloat(invoice.total_amount) || 0;
      summary.byAccount[accountName].received += parseFloat(invoice.amount_received) || 0;
      summary.byAccount[accountName].balance += parseFloat(invoice.balance) || 0;
    });
                            ...
                            <tbody>
                              {Object.entries(summary.byCurrency).map(([currency, data]) => (
                                <tr key={currency}>
                                  ...
                                  <td>{data.balance.toFixed(2)}</td>
                                  <td>
                                    {((data.amount / summary.totalAmount) * 100).toFixed(2)}%
                                  </td>
                                </tr>
                              ))}
                            </tbody>
                              ...
                              <tbody>
                                {filteredData.map(invoice => (
                                  <tr key={invoice.id}>
                                    ...
                                    <td>{parseFloat(invoice.amount_received).toFixed(2)}</td>
